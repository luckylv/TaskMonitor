            ...
            try
            {
                ...
                for (int i = tt.Length - 1; i >= 0; i--)
                {
                    ...
                    if (regStart.IsMatch(tt[i].ToString()))    //正则表达式匹配
                    {
                        ...
                        LogHelper.WriteLogC(i + 1 + "行找到开始匹配，共需要插入" + startResult[1].ToString() + "条数据");
                        ...
                        for (int j = 0; j < Convert.ToInt32(startResult[1].ToString()); j++)
                        {
                            if (regOne.IsMatch(tt[i - j].ToString()))
                            {
                                ...
                                string stationQueryState = SQLHelper.ExecuteScalar<string>(stationQuery, new { Station = result[4].ToString(), Server = Param.Url });
                                ...
                                if (stationQueryState == null)
                                {
                                    ...
                                    SQLHelper.ExecuteNonQuery(insertAWS, new { Station = result[4].ToString(), Server = Param.Url, ValidTime = Convert.ToDateTime(result[1].ToString()), ValidNumber = result[5].ToString(), isWarn = true });

                                }
                                else
                                {
                                    string updateAWS = @"UPDATE " + Param.DBTable + " SET [ValidTime] = '" + Convert.ToDateTime(result[1].ToString()) +
                                                           @"',[ValidNumber] = " + result[5].ToString() +
                                                           " WHERE Station='" + result[4].ToString() + "' and ValidTime<'" + Convert.ToDateTime(result[1].ToString()) + "' and Server='" + Param.Url + "'";
